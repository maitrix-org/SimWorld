...
class CityRenderer(QMainWindow):
    ...
    def draw_frame(self):
        ...
        for element in self.city.elements:
            ...
            size = min(element.bounds.width, element.bounds.height) * 1.4  # 直径为边长的2倍
    ...
    def update_generation(self):
        ...
        if not self.city.is_generation_complete():
            ...
            self.draw_frame()
            #print(f"Roads: {len(self.city.roads)}, Buildings: {len(self.city.buildings)}, Details: {len(self.city.details)}")  # 添加调试信息
        else:
